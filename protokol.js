...
let bobMessageHandler = function(encryptedMessage) {
    ...
    let messageToTrustedPerson = encrypt(bobRandomNumber, bobSessionKey);
    return  messageToTrustedPerson;
};

let isConnectionEstablished = function(id) {
    if(id === (bobRandomNumber-1)) {
        ...
        getElement('bob-verify').addEventListener('click', () => {
            getElement('get-messageA').classList.remove('hidden');
            getElement('alice-get-message').textContent = bobMessageInput.value;
            bobMessageInput.value = '';
        });

        aliceSend.addEventListener('click', () => {
            getElement('get-messageB').classList.remove('hidden');
            getElement('bob-get-message').textContent = aliceMessage.value;
            aliceMessage.value = '';
        });
    }
